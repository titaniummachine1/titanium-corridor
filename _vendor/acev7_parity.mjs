...
for (const depth of [6, 8, 10, 12, 13]) {
  const jsOut = execSync(`node _vendor/acev7_ref_run.js ${depth}`, { cwd: root, encoding: 'utf8' });
  ...
  console.log(`depth ${depth}: ${ok ? 'OK' : 'FAIL'} move=${js.move} score=${js.score} nodes=${js.nodes}`);
