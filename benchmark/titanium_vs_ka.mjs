#!/usr/bin/env node
/**
 * Titanium Rust vs Ka remote.
 *
 *   node benchmark/titanium_vs_ka.mjs
 *   node benchmark/titanium_vs_ka.mjs --games 2 --ka short -v
 */
...
import { KA_VISITS, requestKaMove } from './lib/ka_remote.mjs';
...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i += 1) {
    ...
    } else if (arg === '--ka' && argv[i + 1]) {
      opts.kaPreset = argv[++i];
...
async function playOneGame(titaniumSide, opts) {
  ...
  let plies = 0;
  const stats = { kaMs: 0, titaniumMs: 0, rustMoves: 0 };
  const kaVisits = KA_VISITS[opts.kaPreset] ?? KA_VISITS.short;
  ...
  while (winnerIndex(game) === null && plies < MAX_PLIES) {
    ...
    if (side === titaniumSide) {
      ...
      const started = performance.now();
      const algebraic = await requestKaMove(algebraicHistory, { visits: kaVisits });
      elapsedMs = performance.now() - started;
      stats.kaMs += elapsedMs;
      move = actionToGorisansonMove(parseAlgebraic(algebraic));
      result = { stoppedBy: 'remote', simulations: kaVisits };
    }
    ...
    if (!opts.quiet) {
      if (opts.verbose) {
        printPly({
          ply: plies,
          who: side,
          engine: side === titaniumSide ? 'Titanium' : 'Ka',
          result,
          move,
          elapsedMs,
          game,
  ...
  const replayCode = encodeReplayFromAlgebraic(algebraicHistory, {
    game: 'titanium-vs-ka',
    winner: winner === null ? 'draw' : winner === titaniumSide ? 'Titanium' : 'Ka',
    plies,
  });
...
async function main() {
  ...
  console.log('Rust Titanium vs Ka');
  console.log(`  ${opts.games} games · Ti Rust greedy (genmove) · Ka ${opts.kaPreset} (${kaVisits} visits)`);
  if (!opts.quiet) {
    console.log('  (moves + final board by default · --quiet for replay only · -v --board for board each ply)');
  }
  ...
  for (let i = 0; i < opts.games; i += 1) {
    ...
    if (!opts.quiet) {
      printGameHeader(i + 1, opts.games, titaniumSide);
    }

    const outcome = await playOneGame(titaniumSide, opts);
    totals.kaMs += outcome.stats.kaMs;
    ...
    console.log(formatReplayBlock(outcome.replayCode, { label: `REPLAY game ${i + 1} → web Replay tab` }));
