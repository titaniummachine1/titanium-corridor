...
| Layer | Status |
| ----- | ------ |
| **Movegen** | **Closed.** `wall_masks.rs` shift walls + `ShiftCanStep` pawns. Pawn O1 = branch `movgen-o1-lookup` only. |
...
│   ├── wall_masks.rs      L1/L2/TOPO shift masks (production speedup)
│   └── legal.rs           pawn ShiftCanStep + wall emit + L3 flood
