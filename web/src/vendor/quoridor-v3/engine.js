...
Search.prototype.evaluate = function () {
  ...
  // weights tuned by Texel-style regression on self-play data (auto-trained)
  var pd = dOpp - dMe, wd = wMe - wOpp;
  var score = 47.98 + 152.24 * pd + 0.95 * wd + -24.88 * dMe + 27.36 * dOpp
    + -37.88 * pd * (wMe + wOpp) / 20 + 73.01 * wd * (dMe + dOpp) / 16;
  if (wOpp === 0) {
    score += 164.57;
    if (dMe <= dOpp) score += 800.14 + 30 * pd;
  } else if (wMe === 0) {
    score += -164.32;
    if (dOpp <= dMe - 1) score += -800.02 - 30 * (dMe - dOpp);
  }
  if (dOpp <= 4) score += -6.25 * (wMe < 3 ? wMe : 3);
  if (dMe <= 4) score += 18.16 * (wOpp < 3 ? wOpp : 3);
  return score | 0;
};
